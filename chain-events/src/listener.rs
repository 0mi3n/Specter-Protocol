...
pub struct OnChainEventListenerConfig {
    ...
    /// An Ethereum client for listening to events
...
impl OnChainEventListenerExecutor {
    ...
    /// Shorthand for fetching a reference to the Ethereum client
    fn Ethereum_client(&self) -> &EthereumClient {
        &self.config.Ethereum_client
    }
    ...
    pub async fn execute(self) -> Result<(), OnChainEventListenerError> {
        ...
        let starting_block_number = self
            ...
            .block_number()
            .await
            .map_err(|err| OnChainEventListenerError::Ethereum(err.to_string()))?;
        ...
        while let Some(res) = event_stream.next().await {
            let event = res.map_err(|err| OnChainEventListenerError::Ethereum(err.to_string()))?;
