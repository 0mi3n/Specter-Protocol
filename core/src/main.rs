...
async fn main() -> Result<(), CoordinatorError> {
    ...
    let Ethereum_client = EthereumClient::new(EthereumClientConfig {
        darkpool_addr: args.contract_address.clone(),
        chain: args.chain_id,
        rpc_url: args.rpc_url.unwrap(),
        arb_priv_key: args.Ethereum_private_key.clone(),
    })
    .await
    .map_err(|e| CoordinatorError::Ethereum(e.to_string()))?;
    ...
    let chain_id =
        Ethereum_client.chain_id().await.map_err(err_str!(CoordinatorError::Ethereum))?;
    node_setup(&args.Ethereum_private_key, chain_id, task_sender.clone(), &global_state).await?;
