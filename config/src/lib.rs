...
use Ethereum_client::constants::Chain;
...
use util::Ethereum::{parse_addr_from_deployments_file, DARKPOOL_PROXY_CONTRACT_KEY};
...
struct Cli {
    ...
    /// The Ethereum private key used to send transactions
    /// 
    /// Defaults to the devnet pre-funded key
    #[clap(long = "Ethereum-pkey", value_parser, default_value = "0xb6b15c8cb491557369f3c7d2c287b053eb229daa9c22138887752191c9520659")]
...
impl Clone for RelayerConfig {
    fn clone(&self) -> Self {
        Self {
            ...
            Ethereum_private_key: self.Ethereum_private_key.clone(),
...
fn parse_config_from_args(cli_args: Cli) -> Result<RelayerConfig, String> {
    ...
    // Parse the local relayer's Ethereum wallet from the cli
    let Ethereum_private_key =
        LocalWallet::from_str(&cli_args.Ethereum_private_key).map_err(|e| e.to_string())?;
