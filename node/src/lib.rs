...
use Ethereum_client::client::{EthereumClient, EthereumClientConfig};
...
impl MockNodeController {
    ...
    /// Get a handle to the Ethereum client
    pub fn Ethereum_client(&self) -> EthereumClient {
        self.Ethereum_client.clone().expect("Ethereum client not initialized")
    }
    ...
    /// Add an Ethereum client to the mock node
    pub fn with_Ethereum_client(mut self) -> Self {
        let conf = EthereumClientConfig {
            darkpool_addr: self.config.contract_address.clone(),
            chain: self.config.chain_id,
            rpc_url: self.config.rpc_url.clone().unwrap(),
            arb_priv_key: self.config.Ethereum_private_key.clone(),
        };

        // Expects to be running in a Tokio runtime
        let rt = TokioRuntime::new().expect("Failed to create tokio runtime");
        let client =
            rt.block_on(EthereumClient::new(conf)).expect("Failed to create Ethereum client");
    ...
    pub fn with_task_driver(mut self) -> Self {
        ...
        let Ethereum_client =
            self.Ethereum_client.clone().expect("Tor client not initialized");
        let network_queue = self.network_queue.0.clone();
...
mod test {
    ...
    use reqwest::Method;
    use test_helpers::Ethereum::get_devnet_key;
    ...
    fn test_ping_mock() {
        let conf = RelayerConfig {
            ...
            Ethereum_private_key: get_devnet_key(),
            ..Default::default()
        };
