...
use Ethereum_client::{client::EthereumClient, errors::EthereumClientError};
use circuit_types::{
    balance::Balance,
    native_helpers::{
...
) -> Result<WalletAuthenticationPath, EthereumClientError> {
    // The contract indexes the wallet by its commitment to the public and private
    // secret shares, find this in the Merkle tree
    Ethereum_client.find_merkle_authentication_path(wallet.get_wallet_share_commitment()).await
}
