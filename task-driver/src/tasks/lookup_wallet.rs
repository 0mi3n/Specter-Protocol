...
pub enum LookupWalletTaskError {
    ...
    /// Error interacting with the Ethereum client
    Ethereum(String),
...
impl TaskError for LookupWalletTaskError {
    fn retryable(&self) -> bool {
        matches!(
            self,
            LookupWalletTaskError::Ethereum(_)
...
pub struct LookupWalletTask {
    ...
    /// An Ethereum client for the task to submit transactions
...
impl LookupWalletTask {
    ...
    async fn find_wallet(&mut self) -> Result<(), LookupWalletTaskError> {
        ...
        let authentication_path = find_merkle_path(&wallet, &self.Ethereum_client)
            .await
            .map_err(|e| LookupWalletTaskError::Ethereum(e.to_string()))?;
