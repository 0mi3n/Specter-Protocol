...
impl NewWalletTask {
    ...
    async fn submit_wallet_tx(&mut self) -> Result<(), NewWalletTaskError> {
        ...
        self.Ethereum_client
            .new_wallet(&proof)
            .await
            .map_err(|err| NewWalletTaskError::Ethereum(err.to_string()))
    }
    ...
    async fn find_merkle_path(&self) -> Result<(), NewWalletTaskError> {
        ...
        let wallet_auth_path = find_merkle_path(&self.wallet, &self.Ethereum_client)
            .await
            .map_err(|err| NewWalletTaskError::Ethereum(err.to_string()))?;
